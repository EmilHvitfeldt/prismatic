...
#'
#' @inheritParams color
#' @param shift A numeric between 0 and 1. 0 will do zero saturation, 1 will do
...
clr_saturate <- function(col, shift = 0.5) {
  ...
  if (!all(shift >= 0 & shift <= 1)) {
    stop("`shift` must be between 0 and 1.")
  }
...
#'
#' @inheritParams color
#' @param shift A numeric between 0 and 1. 0 will do zero desaturation, 1 will
#'  do complete desaturation. Defaults to 0.5.
#'
