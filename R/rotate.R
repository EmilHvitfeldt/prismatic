...
#' (Hue-Chroma-Luminance) where the hue of the color will be rotated.
#'
#' @source \url{https://en.wikipedia.org/wiki/HCL_color_space}
#'
#' @inheritParams color
#' @param degrees A number between 0 and 360, denoting the amount of degrees the
...
clr_rotate <- function(col, degrees = 0) {
  ...
  if (!all(degrees >= 0 & degrees <= 360)) {
    stop("`degrees` must be numeric between 0 and 360.")
  }
