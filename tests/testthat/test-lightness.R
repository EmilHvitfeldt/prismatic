test_that("clr_lighten() preserves length", {
...
test_that("clr_darken() preserves length", {
...
test_that("clr_lighten() complains when `col` is wrong", {
  expect_error(clr_lighten("not a color"))
  expect_error(clr_lighten(list(pal = "#000000")))
})

test_that("clr_darken() complains when `col` is wrong", {
...
test_that("clr_lighten() if the length of `shift` isn't 1", {
  ...
  expect_error(
    clr_lighten(rainbow(10), seq(0, 1, length.out = 2)),
    "`shift` must be of length 1 or the same length as `col`."
  )
  expect_error(
    clr_lighten(rainbow(10), seq(0, 1, length.out = 3)),
    "`shift` must be of length 1 or the same length as `col`."
  )
})
...
test_that("clr_darken() if the length of `shift` isn't 1", {
  ...
  expect_error(
    clr_darken(rainbow(10), seq(0, 1, length.out = 2)),
    "`shift` must be of length 1 or the same length as `col`."
  )
  expect_error(
    clr_darken(rainbow(10), seq(0, 1, length.out = 3)),
    "`shift` must be of length 1 or the same length as `col`."
  )
})

test_that("clr_lighten() setting `shift = 0` leaves input unchanged", {
  expect_equal_color(
    clr_lighten(rainbow(10), 0, space = "HSL"), color(rainbow(10)), 0
  )
  expect_equal_color(
    clr_lighten(rainbow(10), 0, space = "HCL"), color(rainbow(10)), 0
  )
  expect_equal_color(
    clr_lighten(rainbow(10), 0, space = "combined"), color(rainbow(10)), 0
  )
})

test_that("clr_darken() setting `shift = 0` leaves input unchanged", {
  expect_equal_color(
    clr_darken(rainbow(10), 0, space = "HSL"), color(rainbow(10)), 0
  )
  expect_equal_color(
    clr_darken(rainbow(10), 0, space = "HCL"), color(rainbow(10)), 0
  )
  expect_equal_color(
    clr_darken(rainbow(10), 0, space = "combined"), color(rainbow(10)), 0
  )
})
...
test_that("setting `shift = 1` leaves result complete black or white", {
  ...
  expect_equal_color(
    clr_lighten(rainbow(10), shift = 1, space = "HCL"),
    color(rep("white", 10)),
    0
  )

  expect_equal(
    clr_darken(rainbow(10), shift = 1, space = "HCL"),
    color(rep("black", 10))
  )

  expect_equal_color(
    clr_lighten(rainbow(10), shift = 1, space = "combined"),
    color(rep("white", 10)),
    0
  )

  expect_equal(
    clr_darken(rainbow(10), shift = 1, space = "combined"),
    color(rep("black", 10))
  )
})
