...
test_that("clr_deutan() if the length of `severity` isn't 1", {
  ...
  expect_error(
    clr_deutan(rainbow(10), seq(0, 1, length.out = 2)),
    "`severity` must be of length 1."
  )
  expect_error(
    clr_deutan(rainbow(10), seq(0, 1, length.out = 3)),
    "`severity` must be of length 1."
  )
  expect_error(
    clr_deutan(rainbow(10), seq(0, 1, length.out = 10)),
    "`severity` must be of length 1."
  )
})
...
test_that("clr_protan() if the length of `severity` isn't 1", {
  ...
  expect_error(
    clr_protan(rainbow(10), seq(0, 1, length.out = 2)),
    "`severity` must be of length 1."
  )
  expect_error(
    clr_protan(rainbow(10), seq(0, 1, length.out = 3)),
    "`severity` must be of length 1."
  )
  expect_error(
    clr_protan(rainbow(10), seq(0, 1, length.out = 10)),
    "`severity` must be of length 1."
  )
})
...
test_that("clr_tritan() if the length of `severity` isn't 1", {
  ...
  expect_error(
    clr_tritan(rainbow(10), seq(0, 1, length.out = 2)),
    "`severity` must be of length 1."
  )
  expect_error(
    clr_tritan(rainbow(10), seq(0, 1, length.out = 3)),
    "`severity` must be of length 1."
  )
  expect_error(
    clr_tritan(rainbow(10), seq(0, 1, length.out = 10)),
    "`severity` must be of length 1."
  )
})
...
test_that("clr_deutan() setting `severity` outside range gives error", {
  expect_error(
    clr_deutan(rainbow(10), severity = -1), "`severity` must be between 0 and 1."
  )
  expect_error(
    clr_deutan(rainbow(10), severity = 2), "`severity` must be between 0 and 1."
  )
})

test_that("clr_protan() setting `severity` outside range gives error", {
  expect_error(
    clr_protan(rainbow(10), severity = -1), "`severity` must be between 0 and 1."
  )
  expect_error(
    clr_protan(rainbow(10), severity = 2), "`severity` must be between 0 and 1."
  )
})

test_that("clr_tritan() setting `severity` outside range gives error", {
  expect_error(
    clr_tritan(rainbow(10), severity = -1), "`severity` must be between 0 and 1."
  )
  expect_error(
    clr_tritan(rainbow(10), severity = 2), "`severity` must be between 0 and 1."
  )
})
...
test_that("plotting returns the data invisibly", {
  res <- expect_invisible(check_color_blindness(c("#D3D5D0", "#76716E")))
  expect_equal(res, c("#D3D5D0", "#76716E"))
})
